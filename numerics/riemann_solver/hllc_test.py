...
def compute_intermediate_wave_speed(rho, u, s, p):
  """Computes the wave speed in the intermediate region."""
  return (
      p[1]
      - p[0]
      + rho[0] * u[0] * (s[0] - u[0])
      - rho[1] * u[1] * (s[1] - u[1])
  ) / (rho[0] * (s[0] - u[0]) - rho[1] * (s[1] - u[1]))


class HLLCTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_compute_wave_speeds_estimates_uniform_field(self, dim):
    ...
    results = hllc._compute_wave_speeds(
        replica_id,
        replicas,
        primitive_neg,
        primitive_pos,
    ...
    match dim:
      case 'x':
        ...
        s_m = testing_utils.to_3d_tensor(u, dim, size, as_tf_tensor=False)
      case 'y':
        ...
        s_m = testing_utils.to_3d_tensor(v, dim, size, as_tf_tensor=False)
      case 'z':
        ...
        s_m = testing_utils.to_3d_tensor(w, dim, size, as_tf_tensor=False)
      case _:
        raise ValueError(
    ...
    with self.subTest(name='s_m'):
      self.assertAllClose(s_m, self.evaluate(results[1]))

    with self.subTest(name='s_r'):
      self.assertAllClose(s_r, self.evaluate(results[2]))

  @parameterized.parameters(*_DIMS)
  def test_compute_wave_speeds_estimates_strong_shock_left(self, dim):
    ...
    s_m = compute_intermediate_wave_speed(rho, u, [s_l, s_r], p)

    with self.subTest(name='s_l'):
      self.assertAllClose(s_l, self.evaluate(results[0]), rtol=types.SMALL)

    with self.subTest(name='s_m'):
      self.assertAllClose(s_m, self.evaluate(results[1]), rtol=types.SMALL)

    with self.subTest(name='s_r'):
      self.assertAllClose(s_r, self.evaluate(results[2]), rtol=types.SMALL)

  @parameterized.parameters(*_DIMS)
  def test_compute_wave_speeds_estimates_strong_shock_right(self, dim):
  ...
  @parameterized.parameters(*_DIMS)
  def test_compute_wave_speeds_estimates_heavyside_function(self, dim):
    ...
    s_m_vec = compute_intermediate_wave_speed(
        [rho_vec[:-1], rho_vec[1:]],
        [u_vec[:-1], u_vec[1:]],
        [s_l_vec, s_r_vec],
        [p_vec[:-1], p_vec[1:]],
    )
    s_m = testing_utils.to_3d_tensor(s_m_vec, dim, size, as_tf_tensor=False)
  ...
  @parameterized.parameters(*_DIMS)
  def test_hllc_convective_flux_correct_quiescent_gas(self, dim):
  ...
  @parameterized.parameters(*_DIMS)
  def test_hllc_convective_flux_correct_plug_flow(self, dim):
  ...
  @parameterized.parameters(*_DIMS)
  def test_hllc_convective_flux_correct_s_l_positive(self, dim):
    ...
    primitive_neg = {
        ...
        types.E: testing_utils.to_3d_tensor(np.ones(nx) * e[0], dim, size),
    ...
    primitive_pos = {
        ...
        types.E: testing_utils.to_3d_tensor(np.ones(nx) * e[1], dim, size),
  ...
  @parameterized.parameters(*_DIMS)
  def test_hllc_convective_flux_correct_s_r_negative(self, dim):
  ...
  @parameterized.parameters(*_DIMS)
  def test_hllc_convective_flux_correct_star_region(self, dim):
    ...
    # is an acoustic wave, and does not perturb the flow in the star region.
  ...
  @parameterized.parameters(*_DIMS)
  def test_hllc_convective_flux_correct_shock_reflection(self, dim):
